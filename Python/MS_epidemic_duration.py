...
##########################################################################################
#write csv
writer = csv.writer(open('Epidemic_duration_T0.2.csv','wb'))
header = ["Graph", "num_modules", "Qvalue", "Tvalue", "Duration", "SD", "num_samples", "SE"] 
...
for T in Trange:
	...
	for graph in graphtype:
		...
		length[graph][Q][num_modules] = len(duration_list)
	...
	#################################   
	    ...
	    for Q, num_modules in zip(Qrange, num_modules_list):
		elements = [graph, str(num_modules), str(Q), str(T), durdict[graph][Q][num_modules], error[graph][Q][num_modules],  length[graph][Q][num_modules], se[graph][Q][num_modules]]
